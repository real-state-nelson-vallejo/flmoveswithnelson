...
export async function syncPropertiesAction(filters: { zone?: string | undefined; minBeds?: number | undefined; maxPrice?: number | undefined; propertyType?: string | undefined }, limit: number = 20) {
    try {
        if (!process.env.BRIDGE_SERVER_TOKEN) {
             throw new Error("BRIDGE_SERVER_TOKEN is not configured. Go to settings or .env to add it.");
        }
        
        const result = await syncBridgePropertiesUseCase.syncProperties(filters, limit);
