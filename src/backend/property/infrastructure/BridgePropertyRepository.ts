...
export class BridgePropertyRepository {
    ...
    async getActiveProperties(filters: { zone?: string | undefined; minBeds?: number | undefined; maxPrice?: number | undefined; propertyType?: string | undefined } = {}, limit: number = 20, skip: number = 0): Promise<{ property: Property, externalId: string }[]> {
        const filterParts = [`StandardStatus eq 'Active'`];
        if (filters.zone) {
            filterParts.push(`(City eq '${filters.zone}' or PostalCode eq '${filters.zone}')`);
        }
        if (filters.minBeds) {
