    ...
    return (
        <div className="w-full max-w-[1400px] mx-auto space-y-6">
            <div className="flex flex-col xl:flex-row gap-8 min-h-[700px] lg:h-[calc(100vh-180px)] p-1">
                {/* Editor Column */}
                <div className="flex-[1.4] flex flex-col gap-6 p-6 md:p-8 border border-white/40 rounded-[2.5rem] bg-white/70 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.05)] overflow-y-auto custom-scrollbar">
                    <div className="flex items-center justify-between pb-6 border-b border-slate-100/50">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center shadow-lg shadow-indigo-500/30 ring-4 ring-indigo-50/50">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-700">Agent Persona</h2>
                                <p className="text-sm text-slate-500 font-medium">Fine-tune behavior and capabilities</p>
                            </div>
                        </div>
                        ...
                        <button
                            onClick={handleSave}
                            disabled={isSaving}
                            className="relative group overflow-hidden bg-slate-900 text-white px-6 py-2.5 rounded-xl font-semibold transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:scale-100 shadow-lg shadow-slate-200"
                        >
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-full">
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Length:</span>
                                <span className="text-xs font-mono font-bold text-indigo-600">{prompt.length.toLocaleString()}</span>
                                <span className="text-[10px] font-bold text-slate-300">/ 10,000</span>
                            </div>
                        </div>
                        <div className="relative group">
                            ...
                            <textarea
                                className="relative w-full min-h-[320px] p-6 bg-white border border-slate-200 rounded-2xl font-mono text-sm leading-relaxed resize-none focus:outline-none focus:ring-0 transition-all shadow-[inner_0_2px_4px_rgba(0,0,0,0.03)] placeholder:text-slate-300"
                                value={prompt}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Model Settings */}
                        <div className="flex flex-col gap-6 p-6 rounded-2xl bg-slate-50/50 border border-slate-100">
                            <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Intelligence Engine</h3>

                            <div className="space-y-4">
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-bold text-slate-600">Active Model</label>
                                    <div className="relative">
                                        <select
                                            className="w-full appearance-none p-3.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all cursor-pointer hover:border-indigo-300 pr-10"
                                            value={modelName}
                                ...
                                <div className="flex flex-col gap-4 p-4 bg-white border border-slate-100 rounded-xl">
                                    <div className="flex justify-between items-center">
                                        <label className="text-sm font-bold text-slate-600">Temperature</label>
                        ...
                        {/* Capabilities */}
                        <div className="flex flex-col gap-4 p-6 rounded-2xl bg-slate-50/50 border border-slate-100">
                            <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Enabled Tools</h3>
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex-1">
                                            <h4 className={`text-sm font-bold ${tools.includes('property_search') ? 'text-slate-900' : 'text-slate-500'}`}>Property Search</h4>
                                            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-tighter">Inventory Access</p>
                                        </div>
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex-1">
                                            <h4 className={`text-sm font-bold ${tools.includes('agenda') ? 'text-slate-900' : 'text-slate-500'}`}>Agenda Management</h4>
                                            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-tighter">Scheduling Tools</p>
                                        </div>
                ...
                {/* Playground Column */}
                <div className="flex-1 flex flex-col gap-6 p-8 border border-slate-200 rounded-3xl bg-slate-900 shadow-2xl overflow-hidden">
                    <div className="flex items-center justify-between pb-6 border-b border-white/10">
                        <div className="flex items-center gap-3">
                            <div className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse ring-4 ring-green-500/20"></div>
                            <h2 className="text-xl font-bold text-white tracking-tight">Test Playground</h2>
                        </div>
                        <button
                            onClick={() => setChatHistory([])}
                            className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] hover:text-white transition-colors"
                        >
                            Reset Session
                        </button>
                    </div>
                        ...
                        {isGenerating && (
                            <div className="flex justify-start animate-pulse">
                                <div className="bg-white/5 border border-white/10 rounded-2xl rounded-tl-none p-4 flex gap-2">
                                    ...
                                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-300 animate-bounce [animation-delay:-0.3s]"></div>
                                </div>
                            </div>
                        )}
                        ...
                        {debugLogs.length > 0 && (
                            <div className="mt-4 border-t border-white/10 pt-2">
                                <button
                                    ...
                                    className="text-xs text-slate-500 hover:text-indigo-400 flex items-center gap-1 w-full text-left font-mono"
                                >
                                    {showDebug ? 'Hide Debug Info' : 'Show Debug Info'}
                                </button>
                                {showDebug && (
                                    <div className="mt-2 text-[10px] font-mono text-green-400 bg-black/40 p-3 rounded-lg border border-white/5 overflow-x-auto">
                                        {debugLogs.map((log, i) => (
                                            <div key={i} className="mb-1 last:mb-0 border-b border-white/5 last:border-0 pb-1 last:pb-0">
                    ...
                    <div className="relative pt-4 border-t border-white/10">
                        <input
                            className="w-full p-4 pl-6 pr-12 bg-white/5 border border-white/10 rounded-2xl text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all text-sm"
                            value={input}
                        ...
                        <button
                            ...
                            className="absolute right-3 bottom-3 p-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 disabled:opacity-30 transition-all group active:scale-90 shadow-lg shadow-indigo-500/20"
                        >
